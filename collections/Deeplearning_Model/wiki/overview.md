...
This collection now has forty-seven deeply ingested anchors and is beginning to connect single-cell generative modeling, atlas-scale generative pretraining, rank-value network-biology transfer, read-depth-aware foundation embeddings, continuous expression-value projection, retention-based single-cell scaling, retrieval-style cell atlas foundation models, transcriptome-as-language modeling, language-model-based transcriptomic reasoning, lightweight LLM-derived metadata embedding reuse, uncertainty-aware hierarchical annotation, neural optimal-transport perturbation modeling, conditional Monge-Gap transport, autoencoder–Schrödinger-bridge time-series dynamics, foundational Schrödinger-bridge theory, biophysical stochastic gene-regulatory probability flows, Gaussian-process-based perturbation regression, biologically motivated mature-RNA foundation modeling, combinatorial perturbation generalization, Bayesian RNA-velocity dynamics, reinforcement-learning-based fate-decision analysis, clinical reinforcement-learning translation for dynamic treatment regimes, diffusion-based stimulus-response generation, AI-virtual-cell roadmapping, broader multimodal human health modeling, paired, unpaired, and mosaic single-cell multi-omics integration, continual bounded-memory multimodal atlas updating, assay-specific RNA-ATAC and RNA-protein generative modeling, protein-background decomposition, multi-hop mosaic reference projection, classical-to-deep multi-omics method taxonomy, VAE loss and regularization design, explicit mask-conditioned multimodal imputation, valid statistical inference after molecular imputation, cross-modal regulatory inference, cell-type-specific enhancer-gene mapping, DNA-sequence-to-single-cell profile prediction, explicit multimodal foundation-model blueprints for molecular cell biology, multiscale transformer architecture proposals, perturbation-atlas roadmaps for causal biology, atlas-based transcriptomic and clonal-lineage fidelity benchmarking, developmental multi-omic atlas substrates, disease-comparative developmental neocortex regulatory atlases, region-specific developmental trajectory maps, sex-stratified psychiatric transcriptomic burden, and perturbation-trained single-cell scaling for oncology. The normalized raw-source ingest backlog is now effectively exhausted except for `.DS_Store`, so the collection has moved from source accumulation into a synthesis-and-lint phase.
...
- Conditional transport across drug and dose contexts, including an explicit distinction between global condition-parameterized maps and one-map-per-perturbation designs.
- Schrödinger-bridge path inference for time-series single-cell snapshots, including unbalanced population mass, reference-process assumptions, and its relation to diffusion modeling.
- Simulation-free probability-flow fitting of Fokker–Planck gene-regulatory dynamics, with separate checks for interpolation, mechanistic regulatory recovery, shifted-condition transfer, counterfactual perturbation, and population mass.
...
- Paired RNA-ATAC generative modeling with asymmetric modality branches and clustering-consistency constraints.
- Paired RNA-protein modeling with explicit cell- and protein-specific antibody-background decomposition.
- Multi-hop mosaic integration through connected pairwise feature-overlap graphs and reference-coordinate projection.
- Continual, bounded-memory multimodal atlas updates that balance adaptation to newly arriving data against retention of historical integration results.
- Multi-omics method selection across correlation, matrix-factorization, probabilistic, kernel, network, reference-projection, and deep-generative families, separated from vertical, horizontal, diagonal, and mosaic data geometry.
- VAE design choices spanning assay-matched likelihoods, mixture- or product-of-experts fusion, MMD, adversarial alignment, cycle consistency, contrastive learning, and latent disentanglement.
- Missing-modality imputation under paired-reference, mosaic-block, feature-link-guided, and explicit mask-conditioned supervision.
- Valid post-imputation inference using observation-propensity correction, flexible outcome models, and doubly robust pseudo-outcomes.
...
- Clonal lineage fidelity as a separate developmental constraint: transcriptomic state matching and a plausible pseudotime path need not preserve progenitor division timing, birth-order logic, or within-clone fate diversity.
...
- Disease-comparative developmental multiome studies that use same-nucleus RNA–ATAC measurements to separate altered cell-state distributions from candidate regulatory programs, while keeping trajectory and TF-target claims appropriately noncausal.
...
- Current anchor systems: [SAVE](entities/SAVE.md), [scGPT](entities/scGPT.md), [scFoundation](entities/scFoundation.md), [Geneformer](entities/Geneformer.md), [CellFM](entities/CellFM.md), [SCimilarity](entities/SCimilarity.md), [Cell2Sentence](entities/Cell2Sentence.md), [C2S-Scale](entities/C2S-Scale.md), [scELMo](entities/scELMo.md), [CellOT](entities/CellOT.md), [CMonge](entities/CMonge.md), [ARTEMIS](entities/ARTEMIS.md), [PFM](entities/PFM.md), [GEARS](entities/GEARS.md), [GPerturb](entities/GPerturb.md), [Orthrus](entities/Orthrus.md), [Cell2fate](entities/Cell2fate.md), [scRL](entities/scRL.md), [Squidiff](entities/Squidiff.md), [AIVC](entities/AIVC.md), [AURORA](entities/AURORA.md), [Tahoe-x1](entities/Tahoe-x1.md), [scMVP](entities/scMVP.md), [totalVI](entities/totalVI.md), [StabMap](entities/StabMap.md), [MIDAS](entities/MIDAS.md), [MIRACLE](entities/MIRACLE.md), [scVAEIT](entities/scVAEIT.md), [GLUE](entities/GLUE.md), [MultiVI](entities/MultiVI.md), [scMODAL](entities/scMODAL.md), [scMultiMap](entities/scMultiMap.md), and [scooby](entities/scooby.md). Supporting reference resource: [HNOCA](entities/HNOCA.md).
...
- Transport modeling now has a sharper split: CMonge learns one context-conditioned endpoint map across drug and dose settings, whereas ARTEMIS uses an unbalanced stochastic bridge to connect time-indexed distributions and infer a continuous path.
- Schrödinger-bridge theory clarifies an important boundary for dynamic single-cell results: a fitted bridge is a reference-process-dependent stochastic coupling, not observed lineage or causal mechanism.
- The collection now separates trajectory interpolation from mechanistic adequacy more explicitly: PFM shows that fitting the same snapshot marginals can still yield different regulatory networks, transfer behavior, knockout predictions, and inferred growth dynamics.
...
- The developing Ts21 neocortex study adds a disease-comparative paired-multiome anchor: it makes cell composition, pseudotime, accessibility, and eRegulon changes jointly inspectable, but still requires perturbation experiments to turn regulatory candidates such as BACH1 into causal mechanisms.
- Mouse cortical-organoid lineage tracing now supplies a complementary reference constraint: population-level transcriptomic similarity can coexist with altered clonal RGP temporal progression, so organoid fidelity needs lineage-resolved tests when a model's claim depends on developmental output rules.
...
- Explicit missingness masks are a modeling choice, not merely bookkeeping: scVAEIT distinguishes structurally absent measurements from observed biological zeros and trains on new random masks to learn conditional completion.
- totalVI adds a second assay-aware lesson: protein counts contain a foreground/background ambiguity that should be modeled per cell and antibody rather than removed by one global cutoff.
- Paired-data specialization can be an advantage rather than a limitation when the scientific task needs same-cell RNA-ATAC structure; scMVP uses asymmetric modality branches instead of forcing both assays through identical networks.
- Multi-hop mosaic integration replaces a global common-feature requirement with graph connectivity, but StabMap shows that sufficiently large and informative bridge datasets are still required.
- Multimodal integration now has a time axis as well as a correspondence axis: MIRACLE wraps MIDAS with bounded rehearsal and dynamic architecture adaptation so a mosaic atlas can incorporate new batches without full reintegration, while making memory sampling, update order, and version provenance central constraints.
- Reference projection is now a distinct algorithmic branch in this collection: it can be faster and more inspectable than a deep generator while making reference choice and path-wise error explicit.
- Integration and imputation quality do not settle downstream inferential validity. Coefficients, p-values, and false-discovery rates require separate calibration after predicted molecular values enter an analysis.
- Double robustness protects against misspecification of one nuisance component under stated assumptions; it does not protect an analysis when both the outcome and observation-propensity models fail.
...
- The collection now separates integration geometry from algorithm family. `Mosaic` or `diagonal` describes the correspondences available in the data, whereas `factorization`, `probabilistic`, `network`, or `VAE` describes how those correspondences are modeled.
- Classical integration methods remain meaningful baselines: interpretability, sample efficiency, and explicit uncertainty can matter more than nonlinear generative capacity, and the new review does not identify one universally superior family.
- Shared latent spaces create a recurring preservation tradeoff: stronger cross-modal alignment can attenuate modality-specific signals, so batch mixing, imputation, and biological interpretation require distinct checks.
...
- Forty-seven source pages have been deeply ingested so far.
