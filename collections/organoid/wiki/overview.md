...
- A new PDO-readiness query now turns multimodal specimen handling, line stability, banking, thaw recovery, fidelity, and assay-endpoint design into one treatment-guidance screening rubric instead of leaving them scattered across separate cancer PDO notes.
- A new donor-derived cancer escalation query now separates three often-blurred translational branches: compact ex vivo validation, immune reconstruction, and PDO-X are framed as different stopping points for different missing-readout problems rather than as one default complexity ladder.
...
- A second synthesis now condenses the same query cluster in a narrower way: an assay-escalation and validation playbook now separates maturity rescue, access fixes, coculture, barrier-specific platforms, host validation, and donor-derived cancer escalation into one decision ladder.
- A follow-up concept cleanup has now shortened the adult-platform, functional-assay, and engineering-screening concept pages so they point more cleanly to the escalation playbook instead of restating the same query burst in full.
- A second concept cleanup pass now does the same for the brain-patterning, brain-benchmarking, GI-endoderm, vascularization, self-organization, and multi-lineage pages, so most of `wiki/concepts/` now reads more like a branching map than a parallel mini-review set.
- The index has now been curated around browsing order as well: concepts, queries, and syntheses are grouped into first-stop, branch-specific, and maintenance-oriented sections instead of one flat chronological list.
- The entity layer in the index is now grouped the same way: first-stop entities, platform anchors, engineering tools, readout entities, and validation endpoints are separated so the entry layer is consistent across concepts, queries, syntheses, and entities.
